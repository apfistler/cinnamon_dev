...
class Element(BaseContent):
    def __init__(self, site, element_type, full_path=None, key=None):
        ...
        self.site = site
        self.file_structure_type = site.get('file_structure_type')
        ...
        self.base_site_directory = site.get('base_site_directory')[self.file_structure_type]
        self.site_directory = site.get('site_directory')[self.file_structure_type] 

        if full_path:
            # Determine the correct site directory based on file_structure_type
            target_site_directory = self.site.get('base_site_directory')[self.file_structure_type]

            # Check if the relative path matches the target site directory
            if self.base_site_directory != target_site_directory:
                raise ValueError(f"The provided full_path '{full_path}' does not belong to the specified file_structure_type '{file_structure_type}'.")
        else:
    ...
    @staticmethod
    def get_element_path(site, file_structure_type, element_type):
        ...
        site_directory = site.get('site_directory')[file_structure_type]
        return os.path.join(site_directory, sub_directory)

    def _get_element_path(self, element_type):
        return Element.get_element_path(self.site, self.file_structure_type, element_type)
